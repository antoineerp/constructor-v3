<script>
  ...
  // Variables pour l'aperçu sécurisé (client-side uniquement)
  ...
  // 🔒 Fonctions d'aperçu supprimées pour sécurité - remplacées par PreviewFrame (client-side)
</script>
...
<div class="p-4 border rounded bg-white shadow-sm mb-6">
  ...
  {#if siteBlueprint}
    ...
    {/if}
    <!-- 🚀 Aperçu sécurisé avec approche Bolt.new (client-side uniquement) -->
    {#if siteFiles && Object.keys(siteFiles).length > 0}
      <div class="mb-4">
        <PreviewFrame 
          files={siteFiles}
          title="Application générée - Exécution sécurisée (Bolt.new style)"
        />
      </div>
      
      <!-- Code du fichier sélectionné (optionnel) -->
      {#if siteSelectedFile && showCodeView}
        <div class="mb-4">
          <div class="flex items-center justify-between mb-2">
            <h4 class="text-sm font-medium text-gray-700">Code source : {siteSelectedFile}</h4>
            <button 
          ...
          <pre class="text-xs bg-gray-50 p-3 rounded max-h-64 overflow-auto border">{siteFiles[siteSelectedFile]}</pre>
        </div>
      {:else if siteSelectedFile}
        <div class="mb-4 text-center">
          <button 
            class="text-xs px-3 py-1 bg-gray-500 text-white rounded hover:bg-gray-600"
            on:click={() => showCodeView = true}
          >
            📝 Voir le code source de {siteSelectedFile}
          </button>
        </div>
      {/if}
    {/if}
