...
export async function POST({ request }) {
  try {
    ...
    try { compile(code, { generate:'ssr' }); ssrOk=true; } catch(e){ diagnostics.push({ phase:'ssr', message:e.message, severity:'error' }); }
    try { compile(code, { generate:'dom' }); domOk=true; } catch(e){ diagnostics.push({ phase:'dom', message:e.message, severity:'error' }); }
