...
export async function POST({ request }) {
  try {
    ...
    try { compile(code, { generate:'ssr', runes:false, compatibility: { componentApi: 4 } }); ssrOk=true; } catch(e){ diagnostics.push({ phase:'ssr', message:e.message, severity:'error' }); }
    try { compile(code, { generate:'dom', runes:false, compatibility: { componentApi: 4 } }); domOk=true; } catch(e){ diagnostics.push({ phase:'dom', message:e.message, severity:'error' }); }
