...
export async function GET({ url, request }) {
  ...
  // Supabase retiré
    ...
    let compiled;
    try { compiled = compile(code, { generate:'ssr', css:'external', runes:false, compatibility: { componentApi: 4 } }); } catch(e){ return json({ success:false, error:'Erreur compilation: '+e.message }, { status:400 }); }
    let Component;
    try {
